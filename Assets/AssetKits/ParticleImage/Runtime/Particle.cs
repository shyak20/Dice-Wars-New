...
namespace AssetKits.ParticleImage {
    public class Particle
    {
        ...
        public IReadOnlyList<TrailPoint> trailPoints => _trailPoints;
        ...
        public void Simulate(float deltaTime)
        {
            ...
            if (_source.trailsEnabled && _trailPoints.Count > 0)
            {
                ...
                if (_trailPoints.Count > 1)
                {
                    ...
                    lastFollowingPoint.point = Vector2.Lerp(_trailPoints[1].point, _lastTrailPoint, Mathf.Abs(_time.Remap(_trailPoints[0].time+_source.trailLifetime, _trailPoints[1].time+_source.trailLifetime, 0f, 1f)));
                ...
                {
                    var trailPoint = _trailPoints[i];
                    Vector2 pointDirection = trailPoint.point;
                    float pointDistance = (i > 0) ? Vector3.Distance(_trailPoints[0].point, trailPoint.point).Remap(0, trailLength, 1f, 0f) : 1f;
                    ...
                    if (_trailPoints.Count > 1)
                    {
                        ...
                        {
                            pointDirection = _trailPoints[i + 1].point - trailPoint.point;
                        }
                        else
                        {
                            pointDirection = trailPoint.point - _trailPoints[i - 1].point;
                        }
                    }
                    
                    
                    Vector2 mid = trailPoint.point; // the mid-point between start and end.
                ...
                if (_time >= _trailPoints[0].time + _source.trailLifetime)
                {
                    ...
                    // Out of range exception fix - Ron
                    if (_trailPoints.Count > 0)
                    {
                        _lastTrailPoint = _trailPoints[0].point;
                    }
                }
