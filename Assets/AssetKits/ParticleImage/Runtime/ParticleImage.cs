        ...
        private void Simulate(float deltaTime, bool prewarming = false)
        {
            ...
            if (isEmitting && !canvasRenderer.cull)
            {
                ...
                if (_bursts != null)
                {
                    ...
                    {
                        var burst = _bursts[i];
                        if (_burstTimer >= burst.time + _startDelay && burst.used == false)
                        {
                            for (int j = 0; j < burst.count; j++)
                            {
                                if (multithreadEnabled)
                                {
                                    ...
                                    {
                                        JobParticles.InsertRangeWithBeginEnd(0, 1);
                                        JobParticles[0] = GenerateJobParticle(j, 0, burst);
                                    }
                                    else
                                    {
                                        JobParticles.Add(GenerateJobParticle(j, 0, burst));
                                    }
#endif
                                }
                                else
                                {
                                    _particles.Add(GenerateParticle(j, 0, burst, 0));
                                }
                            }
            ...
            if (isPlaying)
            {
                ...
                if (multithreadEnabled)
                {
                            ...
                            if (canvas.renderMode == RenderMode.ScreenSpaceCamera)
                            {
                                targetPos = new Vector3(
                                    ((viewportPos.x.Remap(0.5f, 1.5f, 0f, canvasRect.rect.width) -
                                      ...
                                      canvasRect.localPosition.x) / transform.lossyScale.x) * canvasRect.localScale.x,
                                    ((viewportPos.y.Remap(0.5f, 1.5f, 0f, canvasRect.rect.height) -
                                      canvasRect.InverseTransformPoint(transform.position).y +
                                      canvasRect.localPosition.y) / transform.lossyScale.y) * canvasRect.localScale.y,
                                    0);
                            }
                            else
                            {
                                targetPos = new Vector3(
                                    (viewportPos.x.Remap(0.5f, 1.5f, 0f, canvasRect.rect.width) -
                                     canvasRect.InverseTransformPoint(transform.position).x) / transform.lossyScale.x *
                                    canvasRect.localScale.x,
                                    (viewportPos.y.Remap(0.5f, 1.5f, 0f, canvasRect.rect.height) -
                                     canvasRect.InverseTransformPoint(transform.position).y) / transform.lossyScale.y *
                    ...
                    if (space == Simulation.World)
                    {
                        ...
                        _transformDeltaRotation = Quaternion.Inverse(transform.rotation).eulerAngles -
                    ...
                    if (JobParticles.Length > 0)
                    {
                        ...
                        while (lastParticle.time >= lastParticle.lifeTime && (!trailsEnabled ||
                                                                              (TrailData.ElementAt(JobParticles.Length -
                                                                                  1).Length <= 1 || _dieWithParticle)))
                        {
                ...
                var vertices = new NativeArray<Vector3>(_particles.Count * 4, Allocator.Temp);
                var triangles = new NativeArray<int>(_particles.Count * 6, Allocator.Temp);
                var uvs = new NativeArray<Vector2>(_particles.Count * 4, Allocator.Temp);
                var colors = new NativeArray<Color>(_particles.Count * 4, Allocator.Temp);
                ...
                for (int i = _particles.Count - 1; i >= 0; i--)
                {
                    ...
                    if (particle.TimeSinceBorn > particle.Lifetime || prewarming)
                    {
                        continue;
                    }

                    vertices[vert] = particle.points[0];
                    vertices[vert + 1] = particle.points[1];
                    vertices[vert + 2] = particle.points[2];
                    vertices[vert + 3] = particle.points[3];
                    ...
                    uvs[vert] = sheetsArray[particle.GetSheetId].size;
                    uvs[vert + 1] = new Vector2(sheetsArray[particle.GetSheetId].pos.x,
                        sheetsArray[particle.GetSheetId].size.y);
                    uvs[vert + 2] = sheetsArray[particle.GetSheetId].pos;
                    uvs[vert + 3] = new Vector2(sheetsArray[particle.GetSheetId].size.x,
                        sheetsArray[particle.GetSheetId].pos.y);

                    colors[vert] = particle.Color;
                    colors[vert + 1] = particle.Color;
                    colors[vert + 2] = particle.Color;
                    colors[vert + 3] = particle.Color;
                ...
                for (int i = _particles.Count - 1; i >= 0; i--)
                {
                    // Possible argument out of range exception fix - Ron
                    ...
                    if (particle.TimeSinceBorn > particle.Lifetime &&
                        (particle.trailPoints.Count <= 1 || _dieWithParticle))
                    {
                        OnAnyParticleFinished();
