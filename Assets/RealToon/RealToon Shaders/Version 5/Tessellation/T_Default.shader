...
//DDTVRC_LV_F#include "../../../../VRC Light Volumes/Shaders/LightVolumes.cginc"
//DDTVRC_LV_F2#include "Packages/red.sim.lightvolumes/Shaders/LightVolumes.cginc"
			...
			float4 frag(VertexOutput i, float facing : VFACE) : COLOR 
			{
				...
				#if N_F_NLASOBF_ON
					float3 lightColor = lerp(0,DirLigCol,isFrontFace);
				#else
                ...
                float3 halfDirection = normalize(viewDirection+lightDirection);
					...
					half3 RTD_SON_ON_OTHERS = lerp(normalDirection,-normalize(_XYZPosition.xyz - i.posWorld.rgb),RTD_SON_VCBCSON_OO);
				...
				half3 RTD_GLO_OTHERS = RTD_GLO;
					...
					half3 RTD_RL_MAIN = lerp(float3(node_4353,node_4353,node_4353),(RTD_RL_LARL_OO *_RimLightColorPower),smoothstep( 1.71, RTD_RL_S_Sli, pow(1.0-max(0,dot(normalDirection, float3(viewDirection.x + (1.0 - _RimLigPosi.x),viewDirection.y + (1.0 - _RimLigPosi.y),viewDirection.z + (1.0 - _RimLigPosi.z)) )),(1.0 - _RimLightUnfill)) ));
				...
				half3 RTD_ST_OFF_OTHERS = (RTD_RL_CHE_1*RTD_SON_CHE_1*lightColor.rgb);
					...
					//
						...
						half3 RTD_ST_IS_ON = lerp(node_338,float3(node_5738,node_5738,node_5738),smoothstep( RTD_ST_H_Sli, node_3187, (_ShowInAmbientLightShadowThreshold*_ShadowT_var) )); 
            				...
            				RTD_ST_IS = lerp(1,RTD_ST_IS_ON,_ShowInAmbientLightShadowIntensity);
				...
				half3 RTD_R_OFF_OTHERS = ( lerp( RTD_TEX_COL , _MainTex_var.rgb , _MCIALO)  * lerp((RTD_LAS*RTD_LVLC*RTD_ST_IS),RTD_ST,RTD_SS)); 
					...
					float node_3622 = 0.0;
					float node_1766 = 1.0;
					half3 RTD_SL_OFF_OTHERS = (AL_GI( lerp(float3(node_3622,node_3622,node_3622),float3(node_1766,node_1766,node_1766),RTD_GI_FS_OO),i.posWorld.xyz)*_EnvironmentalLightingIntensity);
					...
					half3 RTD_SL_MAS = (half3)0.0;
					half3 RTD_R_SEL = (half3)0.0;
					#ifdef N_F_SLMM_ON
					
						RTD_SL_MAS = lerp(RTD_SL_OFF_OTHERS,(_SelfLitColor.rgb*_SelfLitPower),_MaskSelfLit_var.rgb);
						RTD_R_SEL = lerp(RTD_R,lerp(RTD_R,RTD_TEX_COL*_TEXMCOLINT,_MaskSelfLit_var.rgb),_SelfLitIntensity);
					#else
						half3 RTD_SL_HC_OO = lerp( 1.0, RTD_TEX_COL, _SelfLitHighContrast );
						RTD_SL_MAS = lerp(RTD_SL_OFF_OTHERS,((_SelfLitColor.rgb * RTD_TEX_COL * RTD_SL_HC_OO)*_SelfLitPower),_MaskSelfLit_var.r);
						RTD_R_SEL = lerp(RTD_R,lerp(RTD_R,RTD_TEX_COL*_TEXMCOLINT,_MaskSelfLit_var.r),_SelfLitIntensity);
					#endif
...
//DDTVRC_LV_A#include "../../../../VRC Light Volumes/Shaders/LightVolumes.cginc"
//DDTVRC_LV_A2#include "Packages/red.sim.lightvolumes/Shaders/LightVolumes.cginc"
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
				...
				#if N_F_NLASOBF_ON
					float3 lightColor = lerp(0,DelLigCol,isFrontFace);
				#else
		...
		Pass {
            ...
            VertexOutput vert (VertexInput v) 
			{
				...
				_OEM = lerp(v.normal,normalize(v.vertex),_OutlineExtrudeMethod)*_OutResi;

				half RTD_OL = ( RTD_OL_OLWABVD_OO*0.01 )*_OutlineWidthControl_var.r*lerp(1.0,node_1283,_NoisyOutlineIntensity);

				#if N_F_PA_ON
					o.pos = mul(RT_PA(objPos), float4((v.vertex.xyz + _OutlineOffset.xyz * 0.01) + _OEM * RTD_OL,1) ) + (float4(0,0,_ObjePosiZCS,0.0)* 0.0001);
				#else
					o.pos = UnityObjectToClipPos( float4((v.vertex.xyz + _OutlineOffset.xyz * 0.01) + _OEM * RTD_OL,1) ) + (float4(0,0,_ObjePosiZCS,0.0)* 0.0001);
				#endif
			...
			float4 frag(VertexOutput i) : COLOR 
			{
				...
				#ifdef N_F_LLI_ON
					float3 OutLigCol = clamp(_LightColor0.rgb,_LLI_Min,_LLI_Max);
				#else
					float3 OutLigCol = _LightColor0.rgb;
				#endif

				float3 lightColor = OutLigCol;
