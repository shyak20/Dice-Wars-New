...
//LDVRC_LV_F#include "../../../../VRC Light Volumes/Shaders/LightVolumes.cginc"
//LDVRC_LV_F2#include "Packages/red.sim.lightvolumes/Shaders/LightVolumes.cginc"
            ...
            float4 frag(VertexOutput i) : COLOR 
			{
				...
				half shadowMask = 1;
                    ...
                    shadowMask = UNITY_SAMPLE_TEX2D(unity_ShadowMask, i.lightmapUV).r;
                #elif !defined(LIGHTMAP_ON)
                    shadowMask = LIGHT_ATTENUATION(i);
                #endif

                #if defined(SHADOWS_SHADOWMASK)
                    half realtimeShadow = LIGHT_ATTENUATION(i);
                    attenuation = min(realtimeShadow, shadowMask);
                #else
                    attenuation = LIGHT_ATTENUATION(i) * shadowMask;
                #endif
					...
					half3 RTL_SON_CHE_1 = 1;
							...
							half4 lighma_tex = UNITY_SAMPLE_TEX2D(unity_Lightmap, i.lightmapUV);
							bakedGI = DecodeLightmap(lighma_tex) * 4;
							half3 RTL_UAL = UNITY_LIGHTMODEL_AMBIENT.rgb * bakedGI;
						#else
						...
						half3 RTL_UOAL = RTL_UAL;
							...
							half4 lighma_tex = UNITY_SAMPLE_TEX2D(unity_Lightmap, i.lightmapUV);
							bakedGI = DecodeLightmap(lighma_tex) * 4;
							half3 RTL_GI = AL_GI(float3(0,0,0),i.posWorld.xyz) * bakedGI;
						#else
...
//LDVRC_LV_A#include "../../../../VRC Light Volumes/Shaders/LightVolumes.cginc"
//LDVRC_LV_A2#include "Packages/red.sim.lightvolumes/Shaders/LightVolumes.cginc"
		...
		Pass
        {
            ...
            float4 frag_meta2 (v2f_meta i): SV_Target
            {
                ...
                UNITY_INITIALIZE_OUTPUT(UnityMetaInput, o);
					...
					half3 RTL_TEX_COL = _MainTex_var.rgb * _MainColor.rgb;
					half4 _MaskSelfLit_var = tex2D(_MaskSelfLit,TRANSFORM_TEX(i.uv, _MaskSelfLit));
					...
					half3 RTL_SL_MAS = (half3)0.0;
					#ifdef N_F_SLMM_ON
						RTL_SL_MAS = lerp(0,(_SelfLitColor.rgb*_SelfLitPower),_MaskSelfLit_var.rgb);
					#else
						RTL_SL_MAS = lerp(0,((_SelfLitColor.rgb*RTL_TEX_COL*lerp( 1.0, RTL_TEX_COL, _SelfLitHighContrast ))*_SelfLitPower),_MaskSelfLit_var.r);
					#endif
			
					half3 RTL_SL_ON = lerp(0,RTL_SL_MAS,_SelfLitIntensity);
					half3 RTL_SL = RTL_SL_ON * _SelfLitPower;
				#else
					half3 RTL_SL = 0;
					half3 RTL_SL_CHE_1 = 0;
				#endif
		...
		Pass {
            ...
            VertexOutput vert (VertexInput v)
			{
				...
				#if N_F_PA_ON
					o.pos = mul(RT_PA(objPos), float4((v.vertex.xyz + _OutlineOffset.xyz * 0.01) + _OEM * RTL_OL,1) ) + (float4(0,0,_ObjePosiZCS,0.0)* 0.0001);
				#else
					o.pos = UnityObjectToClipPos( float4((v.vertex.xyz + _OutlineOffset.xyz * 0.01) + _OEM * RTL_OL,1) ) + (float4(0,0,_ObjePosiZCS,0.0)* 0.0001);
				#endif
