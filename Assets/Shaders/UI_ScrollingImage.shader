Shader "DiceGame/UI Scrolling Image"
{
    Properties
    {
        [PerRendererData] _MainTex("Image", 2D) = "white" {}
        _Color("Tint", Color) = (1, 1, 1, 1)
        _MovementSpeed("Movement Speed", Vector) = (0, 0, 0, 0)

        [Header(Look)]
        _EmissiveIntensity("Emissive Intensity", Float) = 1
        _NoiseTex("Noise", 2D) = "white" {}
        _NoiseMovementSpeed("Noise Movement Speed", Vector) = (0, 0, 0, 0)
        _ColorA("Color A (noise black)", Color) = (1, 1, 1, 1)
        _ColorB("Color B (noise white)", Color) = (1, 1, 1, 1)

        [KeywordEnum(Normal, Multiply, Color_Burn, Lighten, Screen, Add, Overlay, Soft_Light, Hard_Light, Vivid_Light, Divide, Subtract)]
        _BlendMode("Blend Mode", Float) = 4
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float2 uv = frac(input.uv + _MovementSpeed.xy * _Time.y);
                float2 noiseUv = frac(input.noiseUv + _NoiseMovementSpeed.xy * _Time.y);

                half4 tex = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, uv);
                half4 col = tex * input.color;
                col.a = saturate(Luminance(tex.rgb)) * tex.a * input.color.a;

                half noise = SAMPLE_TEXTURE2D(_NoiseTex, sampler_NoiseTex, noiseUv).r;
                half3 noiseTint = lerp(_ColorA.rgb, _ColorB.rgb, saturate(noise));
                col.rgb = col.rgb * noiseTint * _EmissiveIntensity;
