...
public sealed class ProgressionTrialCompletedPopupView : ProgressionCelebrationPopupViewBase
{
    ...
    GameIconIndexSO ResolveGameIconIndex()
    {
        ...
        // Dice Select scene uses HoverTooltipManager (it has a serialized GameIconIndexSO but doesn't register GameIconCatalog.Active).
        // Read it via reflection so popups can still tint stat values.
    ...
    void BuildBody(PlayerTrialSO trial)
    {
        ...
        BuildRewardRows(trial.completionRewards, iconIndex);
    }
    ...
    void SetupBodyLayout(string description)
    {
        ...
        // Keep root TMP_Text as style template only; actual description is rendered by a child label.
        ...
        // Only configure anchors/layout when we created the container.
    ...
    void TryBindLegacyRelicOrGemRow(GameObject rowGo, Sprite iconSprite, string text, bool isGem)
    {
        ...
        // Icon: pick an Image that looks like an icon.
